...
Файл **`.env` не используется**. В каталоге `./data` на хосте хранятся **`monitor.db`**, **`.session_secret`** (подпись cookie), **`requests.log`** (ротируемый лог: **POST** к приложению; **GET** к UI и **`/api/*`** не пишутся; строки **`scheduler |`** / **`probe |`** / **`probes |`** — работа планировщика и циклов замеров; **`upstream …`** — список моделей у провайдера) и при необходимости сгенерированный ключ Fernet в БД. Копия строк лога дублируется в stdout (`docker logs`).
