...
Без файла `.env`, только при необходимости: `DATABASE_URL`, `MONITOR_DATA_DIR`, `SESSION_SECRET`, `FERNET_KEY`, **`LLMTESTER_PROBE_PARALLEL`** (число потоков для параллельных замеров по разным целям, по умолчанию `12`, макс. `32`).
