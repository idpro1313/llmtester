...
Файл **`.env` не используется**. В каталоге `./data` на хосте хранятся **`monitor.db`**, **`.session_secret`** (подпись cookie), **`requests.log`** (ротируемый лог HTTP: для обычных путей — метод, путь, код, длительность, IP и `user_id`; для **`/api/*`** дополнительно строки **`req:`** / **`resp:`** с телом запроса и ответа до ~16 КиБ на сторону, с маскированием типичных секретных полей в JSON) и при необходимости сгенерированный ключ Fernet в БД. Копия строк лога дублируется в stdout (`docker logs`).
