...
- **Лог запросов**: страница **`/admin/logs`** (после входа) — просмотр хвоста `data/requests.log`.
- **CLI** `benchmark.py` — прежний сценарий замера в файл/консоль (логика в пакете `llm_benchmark`).

## Быстрый старт (Docker)

Файл **`.env` не используется**. В каталоге `./data` на хосте хранятся **`monitor.db`**, **`.session_secret`** (подпись cookie), **`requests.log`** (ротируемый лог входящих HTTP к страницам админки: метод, путь, код, длительность, IP и `user_id`; запросы к **`/api/*`**, **`/static`**, **`/health`**, **`/favicon.ico`** не пишутся; исходящие вызовы к API провайдера — строки **`upstream begin`** / **`upstream GET`** и **`upstream_req`** / **`upstream_resp`**) и при необходимости сгенерированный ключ Fernet в БД. Копия строк лога дублируется в stdout (`docker logs`).
...
> URL в шаблонах провайдеров — **заглушки**; замените на значения из документации Cloud.ru / Yandex / MWS. У шлюза **Cloud.ru** в ответе без ключа указано `WWW-Authenticate: X-API-KEY`: приложение добавляет заголовок **`X-API-KEY`** ко всем исходящим запросам (вместе с `Authorization: Bearer`), иначе запрос к `/v1/models` может долго «висеть» до таймаута.
