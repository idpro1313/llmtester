...
- **Дашборд**: графики по времени и **сводка** с p50/p95/p99 по успешным замерам и долей ошибок; кнопка «Запустить замеры сейчас» — разовый запуск в фоне; блок **планировщика** — старт/стоп периодических замеров (пауза APScheduler) и статус (интервал из настроек, время следующего цикла).
