...
- **Дашборд**: графики по времени и **сводка** с p50/p95/p99 по успешным замерам и долей ошибок; кнопка «Запустить замеры сейчас» ставит цикл замеров **в фоновый поток** (ответ страницы не ждёт LLM). Периодические замеры по интервалу — отдельно, через планировщик в фоне.
...
Файл **`.env` не используется**. В каталоге `./data` на хосте хранятся **`monitor.db`**, **`.session_secret`** (подпись cookie), **`requests.log`** (ротируемый лог входящих HTTP к страницам админки: метод, путь, код, длительность, IP и `user_id`; запросы к **`/api/*`**, **`/static`**, **`/health`**, **`/favicon.ico`** не пишутся; исходящие вызовы к API провайдера — краткие строки **`upstream begin`** / **`upstream models.list …`** (без тела ответа) и при необходимости сгенерированный ключ Fernet в БД. Копия строк лога дублируется в stdout (`docker logs`).
