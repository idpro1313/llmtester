...
- **Дашборд** (`/dashboard`): сводка p50/p95, доли стрима и usage, таблица ошибок, планировщик, экспорт Excel; линейные графики вынесены на **`/dashboard/charts`** (широкая вёрстка, одна колонка, увеличенная высота, легенда справа, обрезка длинных подписей). На дашборде — ссылка «Графики»; справка по метрикам — на странице графиков. Подсказки при наведении на заголовки столбцов сводки. Разовый запуск замеров в фоне; **«Очистить данные замеров»** — `POST /admin/measurements/clear`; **«Скачать Excel»** — `GET /api/metrics/export` с `hours` / `target_id`. **Время** в таблицах и на осях графиков — **московское** (`Europe/Moscow`); в JSON API — UTC с **`Z`**. В БД — UTC (naive в SQLite).
